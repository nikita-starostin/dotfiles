-- helper only used by the telescope visual-selection greps, moved to fff. uncomment to revert.
...
-- local utils = require('telescope.utils') -- only used by file picking, moved to fff_remap.lua

-- file picking moved to fff.nvim (see fff_remap.lua). uncomment to revert.
...
-- greps
-- live_grep moved to fff.nvim (see fff_remap.lua). uncomment to revert.
-- vim.keymap.set('n', '<leader>lg', builtin.live_grep, { desc = 'look for a text in nvim directory' })
-- grep_string moved to fff.nvim (see fff_remap.lua). uncomment to revert.
